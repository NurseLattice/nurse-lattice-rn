...
(async () => {
  ...
  assert.equal(stores.get(current).size, 118, "Shell, analytics, roadmap, all 53 lessons and 53 banks must be pre-cached");
  ...
  assert.equal(stores.get(current).size, 118, "Do not cache failed or unknown responses");
  offline = true;
  const quiz = await request("data/quizzes/lesson-53.json?v=0.55.0");
  assert.equal((await quiz.json()).questions.length, 10);
  const page = await request("./", { mode: "navigate" });
  assert.ok((await page.text()).includes("RN Quest v0.55.0"));
