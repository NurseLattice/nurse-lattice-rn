...
(async () => {
  ...
  assert.equal(stores.get(current).size, 174, "Shell, analytics, roadmap, all 81 lessons and 81 banks must be pre-cached");
  ...
  assert.equal(stores.get(current).size, 174, "Do not cache failed or unknown responses");
  offline = true;
  const quiz = await request("data/quizzes/lesson-81.json?v=0.86.0");
  assert.equal((await quiz.json()).questions.length, 10);
  const page = await request("./", { mode: "navigate" });
  assert.ok((await page.text()).includes("NurseLattice RN Quest v0.86.0"));
