...
(async () => {
  ...
  assert.equal(stores.get(current).size, 122, "Shell, analytics, roadmap, all 55 lessons and 55 banks must be pre-cached");
  ...
  assert.equal(stores.get(current).size, 122, "Do not cache failed or unknown responses");
  offline = true;
  const quiz = await request("data/quizzes/lesson-55.json?v=0.58.0");
  assert.equal((await quiz.json()).questions.length, 10);
  const page = await request("./", { mode: "navigate" });
  assert.ok((await page.text()).includes("NurseLattice RN Quest v0.58.0"));
